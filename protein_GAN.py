...
#this function is implemented through a Lambda layer. it is needed to ensure that the generator outputs one-hot encodings in a differentiable manner.
#essentially, it is "freezing" a probability distribution onto its lowest-energy state, i.e. the entry that initially has the largest value.
#due to the fact that all distributions having a largest entry at the same index will get mapped to the same one-hot vector, this layer might
#impede the gradient, and it will be useful to consider improvements in the future
temp = 0.001
...
class GAN(object):
	...
	def train(self,iterations,batch_size,handicap):
		plot_data = []
		positive_labels = np.array([np.array([0,1]) for i in range(batch_size)])
		negative_labels = np.array([np.array([1,0]) for i in range(batch_size)])
		for cnt in range(iterations):
			...
			if cnt % handicap == 0:
				...
				y_combined_batch = np.concatenate((positive_labels[0:batch_size//2],negative_labels[0:batch_size//2]))
...
plot_data = gan.train(200,100,20)
...
gan.G.save('protein_generator.h5')
gan.stacked_G_D.save('stacked_G_D.h5')
